...
const Paiement = () => {
    async function paiement() {
        try {
            const result = await fetch('http://localhost:3000/yunuspay/start', {
                method: 'POST',
                headers: {
    ...
    function testpaiementStripe() {
        window.open('https://buy.stripe.com/test_aEUcMSaz6h0A8Jq3cc', '_blank');
        window.location.href = '/';
    }

    function paiementStripe() {
        window.open('https://buy.stripe.com/3cs6qGaFmbj8eY04gg', '_blank');
        window.location.href = '/';
    }
                ...
                <form action="" className='flex flex-col align-middle justify-center gap-14 ml-auto mr-auto'>
                    ...
                    <div className='flex flex-row'>
                        <div className='ml-auto mr-auto '>
                            {/* Vrai bouton */}
                            <button className="px-5 py-1.5 bg-grey rounded-3xl text-white" onClick={paiementStripe}>Paiement CB</button>
                        </div>
                        <div className='ml-auto mr-auto '>
                            {/* Bouton de test */}
                            <button className="px-5 py-1.5 bg-grey rounded-3xl text-white" onClick={testpaiementStripe}>Test Paiement CB</button>
                        </div>
                        <div className='ml-auto mr-auto '>
                            {/* Bouton YunusPay */}
                            <button className="px-5 py-1.5 bg-grey rounded-3xl text-white" onClick={paiement}>Paiement YunusPay</button>
                        </div>
                    </div>
